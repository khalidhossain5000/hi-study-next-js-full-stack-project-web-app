...
function AnnouncementComponent({
  ...
  useEffect(() => {
    ...
    childArray.forEach((child) => {
      if (React.isValidElement(child) && (child.type)[EXPANDABLE_CONTENT_SYMBOL]) {
  ...
  return (
    <>
      ...
      {isMounted && hasExpandable && createPortal(<AnimatePresence>
        {isOpen && (
          <motion.div
            ref={dropdownRef}
...
export function AnnouncementTag({
  ...
  const tagContent = (
    <span
      ...
      {...props}>
      <span
        className="absolute inset-0 rounded-full bg-foreground/5 opacity-70 pointer-events-none" />
...
(AnnouncementExpandedContent)[EXPANDABLE_CONTENT_SYMBOL] = true;